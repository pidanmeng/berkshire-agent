...
| `packages/plugins/demo`（`@berkshire/plugin-demo`，T3） | **demo 插件**（能力块 A/B/C 一体化） | datasource/… 各插件（首个 client 插件） | **webview 半身随插件包走**：页面/组件 JSX（`src/client/demo*.tsx`，经 `@berkshire/plugin-demo/client` 入口 static import）+ 前端 scoped 样式单一来源 `src/client/styles.ts`（sidecar 半身从这取样式去注册，`client/list` 照常下发）。sidecar 半身 `inject: ['slots','clientModules','log']`；一次注册 `stock-preview.footer` 组件（`demo-fund-flow`）+ `watchlist.toolbar` 组件（`demo-watchlist-toolbar`）+ `analysis.menu` 资金流向页（`demo-money-flow` → `/analysis/money-flow`，各带 scoped 样式）；全部经 `ctx.effect` 包裹并逆序撤销；后端走 `ctx.log` + 静态占位数据（诚实：不走 DuckDB）；Config 有 `enableFooter/enableToolbar/enableMenu` 开关。组件跨包不 import 宿主：`FooterContext`/`ToolbarContext` 为宿主 `slots/types.ts` 的结构镜像（v2 共享类型层债务） |
...
| 样式令牌层（第一优先落地） | `@berkshire/theme` + 中枢/插件样式治理 | 样式/主题治理 | [packages/theme](../packages/theme/src/index.ts)（无 `@berkshire/core` 依赖的令牌单一事实源：`THEME_TOKENS` 注册表 / `LIGHT_PALETTE` `DARK_PALETTE` / `bkVar` `bkVarName` / `themeRootCss()`）；中枢 `App.css` 与插件 `styles.ts` 只写 `var(--bk-*)` 引用（**禁魔法色值/间距/圆角**）；webview [src/theme/](../apps/berkshire-agent/src/theme/)（`install.ts` 渲染前注入 `<style data-bk-theme>`、`ThemePalettePage.tsx` 主题对照页核心路由 `/theme`）；魔法色值 lint [packages/theme/scripts/lint-styles.ts](../packages/theme/scripts/lint-styles.ts)；`packages/theme/test/theme.test.ts`。`ctx.theme` 换肤缝、Tailwind 取舍、`bk://` 基底留 v-next |
