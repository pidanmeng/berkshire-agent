...
| | `ctx.slots` / `ctx.clientModules`（能力缝 **Definition**，T1 最小件） | sidecar 侧 slot 占用 + client 插件图 / 前端 bundle 声明 | `Slots`/`ClientModules` extends `Service`：运行时校验（未知 slot/重复 id，fail-closed）+ `client/changed` 广播 + 可逆 disposer；`ClientModuleId` 品牌化；slot 名与 webview `slots/types.ts` 两端一致（共享类型层 v2）。路由语义（任意 slot 的 title/route、静态/核心碰撞/全局 path 唯一校验）已由路由契约化落地（见下行），`bk://` 远程 bundle 留 v-next |
...
| `packages/plugins/demo`（`@berkshire/plugin-demo`，T3） | **demo 插件**（能力块 A/B/C 一体化） | datasource/… 各插件（首个 client 插件） | **webview 半身随插件包走**：页面/组件 JSX（`src/client/demo*.tsx`，经 `@berkshire/plugin-demo/client` 入口 static import）+ 前端 scoped 样式为 **CSS Modules**（作者源 `src/client/*.module.css` → 插件打包阶段编译 `scripts/compile-styles.ts` 产出 `styles.generated.ts`；`.tsx` 拿哈希类名、sidecar 拿注入 css 经 `client/list` 下发，详见下行 P3）。sidecar 半身 `inject: ['slots','clientModules','log']`；一次注册 `stock-preview.footer` 组件（`demo-fund-flow`）+ `watchlist.toolbar` 组件（`demo-watchlist-toolbar`）+ `analysis.menu` 资金流向页（`demo-money-flow` → `/analysis/money-flow`，各带 scoped 样式）；全部经 `ctx.effect` 包裹并逆序撤销；后端走 `ctx.log` + 静态占位数据（诚实：不走 DuckDB）；Config 有 `enableFooter/enableToolbar/enableMenu` 开关。组件跨包不 import 宿主：`FooterContext`/`ToolbarContext` 为宿主 `slots/types.ts` 的结构镜像（v2 共享类型层债务） |
...
| 样式令牌层（第一优先落地 · 已对齐 dsh 两层令牌） | `@berkshire/theme` + 中枢/插件样式治理 | 样式/主题治理 | [packages/theme](../packages/theme/src/index.ts)（无 `@berkshire/core` 依赖的令牌单一事实源：**取值层 `STATIC_TOKENS`（`--bk-static-*`，唯一写实值）+ 别名层 `THEME_TOKENS`（`--bk-*`，组件/插件唯一引用入口，值为 `var(--bk-static-*)`）**；`LIGHT_PALETTE` `DARK_PALETTE` / `bkVar` `bkVarName` / `bkStaticVar` / `themeRootCss()` 两层出口）；中枢 `App.css` 与插件样式只写 `var(--bk-*)` 别名引用（**禁魔法色值**；间距/圆角/字号统一走 `--bk-space-*`/`--bk-radius-*`/`--bk-font-size-*`，host 与插件同源引用，正式去 token 化 v-next）；暗色单表 `[data-theme='dark']`/`prefers-color-scheme` 只覆盖取值层、组件零 `[data-theme]` 选择器；webview [src/theme/](../apps/berkshire-agent/src/theme/)（`install.ts` 渲染前注入 `<style data-bk-theme>`、`ThemePalettePage.tsx` 主题对照页核心路由 `/theme`，色板格为 host 侧 CSS Modules + clsx 组件 `TokenSwatch`）；魔法色值 lint [packages/theme/scripts/lint-styles.ts](../packages/theme/scripts/lint-styles.ts)（认得取值层实值、别名层只许 `var()`，扫描面含插件 `.module.css`）；`packages/theme/test/theme.test.ts`。`ctx.theme` 换肤缝、Tailwind 取舍、`bk://` 基底、spacing/radius/font-size 正式去 token 化、正式样式文件接管取值层均留 v-next |
| host 组件 CSS Modules + clsx（H2） | host 组件用 `.module.css` + clsx | 样式/主题治理 | [apps/berkshire-agent/src/theme/TokenSwatch.tsx](../apps/berkshire-agent/src/theme/TokenSwatch.tsx)（host 侧 `.module.css` 示范：类名 camelCase、状态类由 `clsx` 挂载、`className` 透传、禁 `composes`、禁魔法值）· [TokenSwatch.module.css](../apps/berkshire-agent/src/theme/TokenSwatch.module.css) · [css-modules.d.ts](../apps/berkshire-agent/src/css-modules.d.ts)（通配 declare）。动态样式走 CSS 变量桥（JS 只写变量、规则留 CSS） |
...
| `apps/berkshire-agent/src/slots/*`（T0） | webview 本地 slot 宿主 | sidecar `ctx.slots` 能力缝 | `SlotRegistry` 运行时校验（id 格式/重复 id/API version/未知 slot，fail-closed）+ 按 `order ?? 100` 排序 + 可逆 disposer + `subscribe`/`getSnapshot`（`useSyncExternalStore` 反应式订阅）；`ExtensionSlot` 按槽渲染、每组件包 `ExtensionBoundary`（compact 降级 null 否则横幅）；`types.ts` 类型化 context map + 空状态渲染。T1 后经 `ClientModuleHost` 汇入 sidecar 快照。router/store/`bk://` 仍目标态 |
...
- **范围**：核心脊已落 sessions/log、capabilities、notifier(seam)、slots、clientModules 五条；前端 **webview 本地 slot 宿主（`apps/berkshire-agent/src/slots/*`，T0 最小件）已落地**、**T1 的 sidecar 侧 `ctx.slots`/`ctx.clientModules` 能力缝最小件 + `client/list`/`client_list` + webview `ClientModuleHost`（client 插件图最小链路）已落地**、**路由契约化（任意 slot 的 `route` → 动态导航/路由/页面，core `Slots.routes()`/`CORE_ROUTE_PATHS` + sidecar `routes/list` + Rust `routes_list` + webview `HashRouter`/`src/routes/*`）已落地**、**T3 的 demo 插件（`@berkshire/plugin-demo`，能力块 A/B/C 一体化：footer 组件 + toolbar 组件 + 资金流向页 + scoped 样式，`packages/bundle/{demo,demo-off}` 开关）已落地**（见 §6/§8）；database/datasets/market/scheduler 及正式 router/store 仍为目标态。
