...
| client 插件图最小链路（T1） | [src/client/](../apps/berkshire-agent/src/client/)（`ClientModuleHost.tsx`/`loader.ts`/`demoMinimal.tsx`，本地模块映射 + `<style data-bk-module>` scoped 注入）· [packages/core/src/services/{slots,clientModules}.ts](../packages/core/src/services/clientModules.ts)（`ctx.slots`/`ctx.clientModules` Definition + `client/changed`）· sidecar `client/list` · Rust `client_list` command | + `bk://` 远程 bundle / HMR / 生产样式分发 |
| 动态菜单/页面（T2，能力块 B） | [src/menu/](../apps/berkshire-agent/src/menu/)（`menuStore`/`MenuSync`/`ExtensionRoute`，`useSyncExternalStore` 反应式）· [src/main.tsx](../apps/berkshire-agent/src/main.tsx)（`HashRouter`）+ [src/App.tsx](../apps/berkshire-agent/src/App.tsx)（核心路由 `/`、`/settings` + 动态 `analysis.menu` 路由）· [packages/core/src/services/slots.ts](../packages/core/src/services/slots.ts)（`Slots.menu()`/`CORE_ROUTE_PATHS` 校验）· sidecar `menu/list` · Rust `menu_list` command | **已知限制（最小件）：`ExtensionRoute` 每页渲染 `analysis.menu` 槽的并集、不按 `menu.id` 筛选——多菜单项时页面内容互串**，见 [src/menu/ExtensionRoute.tsx](../apps/berkshire-agent/src/menu/ExtensionRoute.tsx)，v-next 修；另有 `CORE_ROUTE_PATHS` 与 webview 核心路由双端各一份的一致风险。+ layout/menu 全量注入 / store 作用域 |
...
| 桥接协议 sidecar（T1 落地） | [packages/sidecar/src](../packages/sidecar/src/index.ts) · [examples/smoke.ts](../packages/sidecar/examples/smoke.ts)（含 `client/list`、`menu/list`） | （T3 webview 接线已落地；T4 完整 E2E） |
