...
| 样式治理 / 令牌层（第一优先落地） | [packages/theme](../packages/theme/src/index.ts)（`@berkshire/theme`：**无 `@berkshire/core` 依赖**的令牌单一事实源——`THEME_TOKENS` 注册表 / `LIGHT_PALETTE` `DARK_PALETTE` / `bkVar`/`bkVarName` / `themeRootCss()`；webview [src/theme/install.ts](../apps/berkshire-agent/src/theme/install.ts) 渲染前注入 `<style data-bk-theme>`）· 中枢 [src/App.css](../apps/berkshire-agent/src/App.css) 与插件 [styles.ts](../packages/plugins/demo/src/client/styles.ts) 均只写 `var(--bk-*)` 引用、禁魔法色值 · 主题对照页 [src/theme/ThemePalettePage.tsx](../apps/berkshire-agent/src/theme/ThemePalettePage.tsx)（核心路由 `/theme`）· 魔法色值 lint [packages/theme/scripts/lint-styles.ts](../packages/theme/scripts/lint-styles.ts)（`bun run lint:styles`）· `packages/theme/test/theme.test.ts` | + `ctx.theme` 换肤能力缝（三角色）/ 远程 bundle 基底 / Tailwind 取舍（见 style-governance §3/§4） |
