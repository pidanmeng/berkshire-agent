...
| 应用壳插件（`@berkshire/base-ui`，dark-first 终端风，本轮下沉 + WP-3 优化 + WP-6 设置弹窗） | [packages/plugins/base-ui/src/client/](../packages/plugins/base-ui/src/client/index.tsx)（**grid 三区域骨架** `AppShell`〔左导航 / 右顶栏 / 内容区，`grid-template-areas`〕+ `Sidebar` 可扩展侧边栏 + `StatusBar` 右侧路由区顶状态栏 + `SettingsDialog` **设置弹窗**〔WP-6：`@berkshire/ui` `Modal`，左分组〔通用/模型/插件设置〕+ 右表单；通用 `GeneralSettingsForm` / 模型 `ModelSettingsForm` 骨架用 `@berkshire/ui` Input/Select，模型 Key 字段只存 env 引用名、不落明文，**持久化已接 storage 缝**（WP-7：字段经 `@berkshire/ui-slots` `usePersistedField` 逐项落 `settings` 命名空间 KV；`settings.section` 插件面板经槽 context 取同一 `StorageHandle` 落各自 ns）〕+ 主题对照 `ThemePalettePage`＋`installThemedRoot` 令牌根注入；壳组件 CSS Modules + clsx、`var(--bk-*)`，依赖 `@berkshire/ui-slots`/`react-router-dom`/`clsx`/`@berkshire/ui`，**复用 `@berkshire/ui` Button/Badge/Modal**（不手写平行按钮/状态条），路由与桥接态由宿主 prop 注入；壳帧 `RootShell` 经共享 `root` 槽挂载，`registerRootShell` 注册即效应）· 布局挂点（`@berkshire/ui-slots` `FrontendSlotContextMap`）增 `layout.navigation.extra`/`layout.sidebar.footer`/`layout.statusbar.right`/`settings.cards`/**`settings.section`**（与 core `SLOT_NAMES` 两端同一契约，其中 `settings.cards` context 携带设置分组契约 `SettingsGroup`，`settings.section` 是 **WP-6 设置 Seam**：插件贡献的设置表单面板挂进弹窗「插件设置」分组，每槽包 `ExtensionBoundary`）＋侧边栏按 `RouteDescriptor.section` 分组 · **设置分组契约** `settingsGroups.ts`（`DEFAULT_SETTINGS_GROUPS`＝通用/模型；设置弹窗/表单已由 **WP-6** 落地，`/settings` 路由已移除） · demo 插件证明性注册（导航追加项/状态项/设置卡片/设置表单面板 `DemoSettingsSection`） | + stage `page.header` 每页头槽（v-next）/ shell 折叠态持久化 / store 作用域 / 经 sidecar 装配可 disable（step 3b 余下）+ 插件自定义顶层设置分组 |
