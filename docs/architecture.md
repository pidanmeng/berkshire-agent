...
| 动态路由/页面（能力块 B，路由契约化） | [src/routes/](../apps/berkshire-agent/src/routes/)（`routesStore`/`RouteSync`/`ExtensionRoute`，`useSyncExternalStore` 反应式）· [src/main.tsx](../apps/berkshire-agent/src/main.tsx)（`HashRouter`）+ [src/App.tsx](../apps/berkshire-agent/src/App.tsx)（核心路由 `/`、`/theme` + 插件自声明动态路由；设置是弹窗、非路由）· [packages/core/src/services/slots.ts](../packages/core/src/services/slots.ts)（`Slots.routes()`：**任意 slot** 带 `route` 的声明汇总，含 slot 归属 + `CORE_ROUTE_PATHS`/静态/全局 path 唯一校验）· sidecar `routes/list` · Rust `routes_list` command | **已知限制（最小件）：`ExtensionRoute` 渲染 `route.slot` 槽的并集、不按 `route.id` 筛选——同一个 slot 被多个带 `route` 声明复用时各页内容互串**，见 [src/routes/ExtensionRoute.tsx](../apps/berkshire-agent/src/routes/ExtensionRoute.tsx)，v-next 修；另有 `CORE_ROUTE_PATHS` 与 webview 核心路由双端各一份的一致风险。+ 全量 layout/menu 注入 / store 作用域 |
| 数据源能力缝 + DuckDB + 数据管理页（WP：数据源落地） | core Definition：[seams/dataSources.ts](../packages/core/src/seams/dataSources.ts)（`ctx.dataSources`：按功能分类逐数据集路由，偏好落 storage）· [services/datasets.ts](../packages/core/src/services/datasets.ts)（`ctx.datasets` 注册表 + `BUILTIN_DATASETS`：**S1 数据模型规范化——八组基础数据 + 实时快照、`DatasetDeclaration`（`materialization`/`partition`/`generation`/`columnSchema`/`sync`）、覆盖日期元模型 `DatasetCoverage`**，仅为契约/类型/声明层） · [services/database.ts](../packages/core/src/services/database.ts)（`ctx.database` + `database/dataset-updated` + `DatabaseTableInfo` 物化/分区/generation 可选字段） · 纯数据契约工具 [dataContract.ts](../packages/core/src/dataContract.ts)（八组 `*_COLUMNS` + `columnSqlType`/`forwardAdjustFactor`/`forwardAdjust` **单一实现**，契约测试 [test/](../packages/core/test/)）；sidecar DuckDB Provider（[duckdb-provider.ts](../packages/sidecar/src/duckdb-provider.ts)，`@duckdb/node-api` 惰性打开 `$BK_HOME/berkshire.duckdb`，**Node 侧写路径**）+ 同步编排（[sync.ts](../packages/sidecar/src/sync.ts)，整表替换 + 事件）；协议 `data-sources/{list,candidates,set-preference,probe,sync}` + `database/tables`（[protocol.ts](../packages/sidecar/src/protocol.ts)）；Rust `data_sources_*`/`database_tables` command（[bridge.rs](../apps/berkshire-agent/src-tauri/src/bridge.rs)/[lib.rs](../apps/berkshire-agent/src-tauri/src/lib.rs)）；webview 薄客户端 + `DataManagementApi` 句柄（[api.ts](../apps/berkshire-agent/src/lib/api.ts)/[dataManagementApi.ts](../apps/berkshire-agent/src/lib/dataManagementApi.ts)）；provider 插件 [datasource-fuyao](../packages/plugins/datasource-fuyao/src/provider.ts)（realtime/daily/adj_factor/financial/calendar；minute 声明未落地）/ [datasource-csv](../packages/plugins/datasource-csv/src/provider.ts)（最小第二 provider）；数据管理页 [plugin-data-manager](../packages/plugins/data-manager/src/index.ts)（`data.management` 槽 + 路由 `/data`，页面不自连桥，context 注入 API） | + Rust 单写者（`duckdb-rs`）/ parquet 分区多文件粒度 / minute（quota-h）/ `ctx.credentials`（v-next） |
| 股票日历 + 市场时间（**S2-market-calendar，已落地**） | core [services/marketTime.ts](../packages/core/src/services/marketTime.ts)（`ctx.marketTime`：`CN_TZ` 纪元换算 + `isWeekend`/`isTradingDay` 探针降档链 + TTL 缓存 + `nowIsTradingSession`/`isMarketClosed`/`nextClose`/`nextOpen`，S3 定时任务地基）+ `TradeDayProbe` Provider 契约 · fuyao [provider.ts](../packages/plugins/datasource-fuyao/src/provider.ts)（`calendarRow` 归一 `trade_date/open/close/status` + `createCalendarProbe` 注册交易日探针），`calendar` 经通用 `data-sources/sync` 单写者落库 · 测试 [core/test/marketTime.test.ts](../packages/core/test/marketTime.test.ts) + [fuyao/test/provider.test.ts](../packages/plugins/datasource-fuyao/test/provider.test.ts)（18 用例） | + 启动/收盘同步的**驱动已落地**（S3-sync-autopilot，见下）/ 半天市感知的 `nextClose` 细化 |
| Enriched 数据集 + 技术指标现算（**S2-enriched-ohlev-indicator，已落地**） | core 纯数据契约 [dataContract.ts](../packages/core/src/dataContract.ts)（`isHaltDay`/`filterHaltDays` TSP 停牌语义 + `groupAdjFactors` + `adjustPrice` 前复权单价**唯一实现** + `toEnrichedRows` 原始日K+除权因子→前复权窄表基点列）· 指标纯函数库 [indicators.ts](../packages/core/src/indicators.ts)（sma/ema/macd/boll/kdj/atr/rsiSeries/momentum/annualVolatility，窗口不足 null 不伪造）· `ctx.indicators` 缝（[services/indicators.ts](../packages/core/src/services/indicators.ts)，Definition + `IndicatorsProvider` 契约 + register 扩展，三角色一次交付）· sidecar [enriched.ts](../packages/sidecar/src/enriched.ts)（derived provider `createEnrichedProvider` 读 `daily`+`adj_factor` 变换，`daily`/`adj_factor` 表缺失 fail-closed；注册进 `ctx.dataSources` 常驻 → `data-sources/sync` 对 enriched 走既有单写者写路径 parquet-view/覆盖登记/事件全复用；`computeIndicatorsFromDb` 现算读路径暴露协议 `data-sources/enriched-indicators`）· 测试 [core/test/{enriched,indicators}.test.ts](../packages/core/test/indicators.test.ts) + [sidecar/test/enriched.test.ts](../packages/sidecar/test/enriched.test.ts) | + 分区多文件粒度 / 生成原子发布的运行时 details / `turnover_rate` 等依赖历史股本/递归状态列的填充 / 性能缓存（S3） |
| 新数据组扩展缝 + 覆盖登记（**S2-dataset-extension-seam，已落地**） | core [services/datasets.ts](../packages/core/src/services/datasets.ts)（`ctx.datasets.register(decl)` **staged 注册 + 可逆**：重复 id / 非法·保留字列名 / 非法物化 / 版本不匹配写入前校验、fail 即丢弃不留半注册；卸载即撤销；`registerCoverage`/`recordCoverage`/`reportCoverage` + `datasets/coverage-updated`）· sidecar [sync.ts](../packages/sidecar/src/sync.ts)（写路径收敛到 `DatasetDeclaration.columnSchema`，按声明支持 `embedded`/`parquet-view`（staging→`COPY`→`CREATE OR REPLACE VIEW read_parquet`））· 覆盖 Provider [coverage-provider.ts](../packages/sidecar/src/coverage-provider.ts)（`dataset_coverage` 元表，单写者幂等更新）· 协议 `data-sources/coverage`；参考插件 [datasource-example](../packages/plugins/datasource-example/src/index.ts)（教程 [plugin-development.md §1.5](plugin-development.md#15-加一个新数据组扩展缝s2)）· 测试 [core/test/datasets.test.ts](../packages/core/test/datasets.test.ts) + [sidecar/test/{dataset-extension,sync}.test.ts](../packages/sidecar/test/sync.test.ts) + [plugin/test/extension.test.ts](../packages/plugins/datasource-example/test/extension.test.ts) | + generation 原子发布 / parquet 分区多文件粒度（归 S3） |
| 覆盖日期注册表读 API / 手动重算 / 缺洞自检（**S2-coverage-date-registry，已落地**） | sidecar [coverage-utils.ts](../packages/sidecar/src/coverage-utils.ts)（`scanDatasetTable`/`coverageDateColumnOf`/`rescanAndRecordCoverage`/`coverageGaps`/`coverageGapsFor`/`listCoverageEntries`）· 协议 `data-sources/{coverage,coverage-refresh,coverage-gaps}` + `data-sources/list.coverage`（未登记 fail-closed → `covered:false`）· Rust [bridge.rs](../apps/berkshire-agent/src-tauri/src/bridge.rs)+[lib.rs](../apps/berkshire-agent/src-tauri/src/lib.rs) command · 宿主 [api.ts](../apps/berkshire-agent/src/lib/api.ts)/[dataManagementApi.ts](../apps/berkshire-agent/src/lib/dataManagementApi.ts) + 共享 DTO [ui-slots/types.ts](../packages/ui-slots/src/types.ts) · 页面 [DataManagerPage.tsx](../packages/plugins/data-manager/src/client/DataManagerPage.tsx)「覆盖日期」区 + 重算 · 测试 [sidecar/test/coverage.test.ts](../packages/sidecar/test/coverage.test.ts) | + 交易日语义缺洞（跳过非交易日）/ 缺洞驱动自动补齐（S3） |
| 默认定时任务（**S3-sync-autopilot，已落地：启动同步 + 收盘同步**） | sidecar [autopilot.ts](../packages/sidecar/src/autopilot.ts)（`attachAutopilot` 挂 ready：**启动同步**（ready 后就近同步一次，`ctx.marketTime` 交易日判定，非交易日只刷 `calendar` 不盲拉）+ **收盘同步**（周期重检 `hasReachedClose`＝交易日 && 北京盘中 ≥15:00，`closeDueForToday` 幂等 `lastCloseRunDate` 落 storage）+ 配置 storage ns `autopilot`/key `config`（`parseAutopilotConfig` loud-fail，缺失走内置默认）· `runSyncBatch` 事件 `sync/started`/`sync/completed`/`sync/failed`（@mode emit，events.ts 推送宿主）· 单写者：只调 `runDatasetSync`，覆盖登记复用 S2 seam）· sidecar [scheduler.ts](../packages/sidecar/src/scheduler.ts)（`./autopilot` 别名）· sidecar [index.ts](../packages/sidecar/src/index.ts)（`attachAutopilot` 装配，teardown 逆序清除 tick）· [events.ts](../packages/sidecar/src/events.ts)（relay `sync/*`）· 测试 [sidecar/test/autopilot.test.ts](../packages/sidecar/test/autopilot.test.ts) | + `market/scheduler` 核心服务（core 侧定时任务编排，仍目标态）/ 半天市感知 / 前端调度设置 UI（当前走 storage 偏好 + 文档） |
| 高速读写 / 缓存 / 性能优化（**S3-cache-performance，已落地，Node sidecar 侧**） | sidecar [cache.ts](../packages/sidecar/src/cache.ts)（`DatasetCache`：单调 generation + 读透 `read(scope,key,builder)` 命中复用 / miss → `builder()` 构建新快照**原子替换**写入（构建期 generation 未变才写——并发写不覆盖）· [sync.ts](../packages/sidecar/src/sync.ts) `runDatasetSync(..., cache)` **落库成功 bump generation** + 失效元作用域（coverage 快照）· `database/dataset-updated` payload 新增可选 `generation`（core [services/database.ts](../packages/core/src/services/database.ts)）· 写路径 parquet **原子替换**（`COPY`→`.tmp`→`renameSync`，`replaceWithRetry` Windows 读锁穿透）· 覆盖快照（`data-sources/coverage`/`list.coverage`）与 enriched 读路径（`computeIndicatorsFromDb`/`enrichFromDb`）读透缓存，协议 `data-sources/sync` 传 cache · parquet-view 谓词下推（`read_parquet` 惰性 scan，切片不入全量内存）· DuckDB query 出口 JSON 安全归一（复用 [json-safe.ts](../packages/sidecar/src/json-safe.ts)）· 基准 [sidecar/bench/cache-performance.ts](../packages/sidecar/bench/cache-performance.ts)（`cd packages/sidecar && bun run bench`；Node 侧，`cargo test -p bk-core` 目标态不可用）· 测试 [sidecar/test/{cache, cache-sync}.test.ts](../packages/sidecar/test/cache-sync.test.ts) | + Rust `duckdb-rs` / Rust 内存热缓存（仍目标态）/ generation 跨进程语义 / parquet 分区多文件粒度 |
| 桥接 JSON 安全序列化（S1 修复 bigint，已落地） | [sidecar/src/json-safe.ts](../packages/sidecar/src/json-safe.ts)（单一确定性出口：`bigint` 安全→number、超界→十进制字符串、错误入参 fail-closed 抛 `JsonSafeError`）· 协议层 [protocol.ts](../packages/sidecar/src/protocol.ts)（`serializeResult`/`serializeError`）与事件推送 [events.ts](../packages/sidecar/src/events.ts) 共用 · [duckdb-provider.ts](../packages/sidecar/src/duckdb-provider.ts) `query()`/`tables()` 出口归一（双保险）· 测试 [sidecar/test/serialize.test.ts](../packages/sidecar/test/serialize.test.ts)（另见 [secondary-development.md §6](secondary-development.md#6-已有-vs-目标诚实标注)） | + 无 |
| 数据基座（S1 升权：数据能力改为和 base-ui 相同的宿主绑定基座） | [packages/sidecar/src/index.ts](../packages/sidecar/src/index.ts)（`BASE_DATA_PLUGINS`：fuyao/csv/data-manager 由 sidecar **无条件常驻装配**，不依赖用户 cordis.yml；老配置数据行经 `BASE_DATA_NAMES` **承接去重**、不重复装配；**S3 新数据能力装配落点（目标态骨架）**）· [packages/bundle/base/cordis.patch.yml](../packages/bundle/base/cordis.patch.yml)（**不再列数据插件行**，对齐 base-ui 不入插件表）· [src/onboarding/defaultOptions.ts](../apps/berkshire-agent/src/onboarding/defaultOptions.ts)（数据基座只展示为锁定项、不可勾选；`BASE_DATA_PLUGINS`）· `packages/sidecar/test/process.test.ts` + `examples/smoke.ts`（常驻数据基座断言：`data-sources/list` 含 fuyao/csv、`client/list` 含 data-manager） | + S3 落地 Enriched/覆盖记录/定时任务（进基座装配） |
| 应用壳插件（`@berkshire/base-ui`，dark-first 终端风，本轮下沉 + WP-3 优化 + WP-6 设置弹窗） | [packages/plugins/base-ui/src/client/](../packages/plugins/base-ui/src/client/index.tsx)（**grid 三区域骨架** `AppShell`〔左导航 / 右顶栏 / 内容区，`grid-template-areas`〕+ `Sidebar` 可扩展侧边栏 + `StatusBar` 右侧路由区顶状态栏 + `SettingsDialog` **设置弹窗**〔WP-6：`@berkshire/ui` `Dialog` 复合家族，左分组〔通用/模型/插件设置〕+ 右表单；通用 `GeneralSettingsForm` / 模型 `ModelSettingsForm` 骨架用 `@berkshire/ui` Input/Select，模型 Key 字段只存 env 引用名、不落明文，**持久化已接 storage 缝**（WP-7：字段经 `@berkshire/ui-slots` `usePersistedField` 逐项落 `settings` 命名空间 KV；`settings.section` 插件面板经槽 context 取同一 `StorageHandle` 落各自 ns）〕+ 主题对照 `ThemePalettePage`＋`installThemedRoot` 令牌根注入；壳组件 CSS Modules + clsx、`var(--bk-*)`，依赖 `@berkshire/ui-slots`/`react-router-dom`/`clsx`/`@berkshire/ui`，**复用 `@berkshire/ui` Button/Badge/Dialog**（不手写平行按钮/状态条），路由与桥接态由宿主 prop 注入；壳帧 `RootShell` 经共享 `root` 槽挂载，`registerRootShell` 注册即效应）· 布局挂点（`@berkshire/ui-slots` `FrontendSlotContextMap`）增 `layout.navigation.extra`/`layout.sidebar.footer`/`layout.statusbar.right`/`settings.cards`/**`settings.section`**（与 core `SLOT_NAMES` 两端同一契约，其中 `settings.cards` context 携带设置分组契约 `SettingsGroup`，`settings.section` 是 **WP-6 设置 Seam**：插件贡献的设置表单面板挂进弹窗「插件设置」分组，每槽包 `ExtensionBoundary`）＋侧边栏首页与插件自声明页面**同权扁平**排布（无分组；[sidebarItems.ts](../packages/plugins/base-ui/src/client/sidebarItems.ts)：`SidebarItem` 模型 `id/title/path/order/hidden` + `useSidebarItems` 反应式合并「声明序 + `settings/sidebar.items` 覆盖 + `settings/sidebar.preset` 预设」，排序/隐藏/预设切换 UI 留 v-next 设置弹窗） · **设置分组契约** `settingsGroups.ts`（`DEFAULT_SETTINGS_GROUPS`＝通用/模型；设置弹窗/表单已由 **WP-6** 落地，`/settings` 路由已移除） · demo 插件证明性注册（导航追加项/状态项/设置卡片/设置表单面板 `DemoSettingsSection`） | + stage `page.header` 每页头槽（v-next）/ shell 折叠态持久化 / store 作用域 / 经 sidecar 装配可 disable（step 3b 余下）+ 插件自定义顶层设置分组 + 侧边栏项排序/隐藏/预设切换 UI（契约已留：`settings/sidebar.items` + `settings/sidebar.preset`） |
| demo 插件（T3，能力块 A/B/C 一体化） | [packages/plugins/demo/src/index.ts](../packages/plugins/demo/src/index.ts)（`@berkshire/plugin-demo`：footer + toolbar 组件 + 资金流向页 + 应用壳布局组件 + scoped 样式，`inject: ['slots','clientModules','log']`）· [packages/bundle/demo/cordis.patch.yml](../packages/bundle/demo/cordis.patch.yml)（enable）+ [packages/bundle/demo-off/cordis.patch.yml](../packages/bundle/demo-off/cordis.patch.yml)（disable）· [packages/plugins/demo/examples/smoke.ts](../packages/plugins/demo/examples/smoke.ts)（boot 级 enable/disable 复现）· **webview 半身随插件包走**：[packages/plugins/demo/src/client/](../packages/plugins/demo/src/client/demoFundFlow.tsx)（`DemoFundFlow`/`DemoWatchlistToolbar`/`DemoMoneyFlow` + **应用壳布局组件** `DemoNavExtra`/`DemoStatusItem`/`DemoSettingsCard`/`DemoSettingsSection`（WP-6 设置 Seam 证明：`settings.section` 设置表单面板），前端 scoped 样式为 **CSS Modules**：作者源 `*.module.css` → 插件打包阶段编译 [scripts/compile-styles.ts](../packages/plugins/demo/scripts/compile-styles.ts) 产出 [styles.generated.ts](../packages/plugins/demo/src/client/styles.generated.ts)，`.tsx` 拿哈希类名、sidecar 拿注入 css 经 `client/list` 下发）。**M3 起 host 不再 static import 插件 client**：插件在运行期自报 `CLIENT_ENTRY_URL`（**两态探测**：`./client/index.js`（已构建 dist）落盘命中则用它，否则退回 `./client/index.tsx`（dev 源码态——bun 把 workspace 插件解析到 `src/index.ts`）），sidecar `client/list` 快照携带它，Rust `to_bk_url` 按 `$BK_HOME` 内路径规范化成 `bk:///…`（`$BK_HOME` 外即 dev workspace 插件**原样透传** `file://`），webview [src/client/loader.ts](../apps/berkshire-agent/src/client/loader.ts)（`importClientModule` + **`normalizeClientUrl`**：浏览器内 `file://` → Vite `/@fs/<abs>` serve，`.tsx` 归 Vite Fast Refresh；`bk://`/`http(s)` 原样）运行时 `import(url)` + 具名导出 + scoped 样式注入（[ClientModuleHost.tsx](../apps/berkshire-agent/src/client/ClientModuleHost.tsx)），共享裸名经 [src/lib/sharedImportMap.ts](../apps/berkshire-agent/src/lib/sharedImportMap.ts) import-map 解析（宿主接线点，默认空；dev 下由 Vite 变换解析裸名，prod `bk://` 仍需 import-map 接线）sidecar 装配叠入 demo bundle（`packages/sidecar/src/index.ts`），`process.test.ts` 断言卸除顺序 `demo -> notify-console -> core` | + HMR / 生产样式分发包 / import-map 真实 URL（待运行验证） |
...
| bundle 配置层（v1） | [bundle/base/cordis.patch.yml](../packages/bundle/base/cordis.patch.yml)（核心脊 + notify-console；**S1 升权：数据插件已升为宿主基座、由 sidecar 常驻装配，不再列插件行**）· [bundle/headless/cordis.patch.yml](../packages/bundle/headless/cordis.patch.yml) | + `!!js` / dump-config / isolate（v2） |
