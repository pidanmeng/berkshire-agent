...
> 注意：`@berkshire/cordis` vendor（M4）**已实现**——[packages/cordis](../packages/cordis/src/index.ts) `export *` 重导出官方 `cordis`，全仓 `declare module`/`import` 统一改走 scoped `@berkshire/cordis`（不再裸 `cordis`）；`cordis` 本体仍是运行时底座。历史差异见 [secondary-development.md §8](secondary-development.md#8-v1-落地说明已实现的-headless-最小核心脊)。
...
| 应用壳插件（`@berkshire/base-ui`，Vercel 黑白风，本轮下沉） | [packages/plugins/base-ui/src/client/](../packages/plugins/base-ui/src/client/index.tsx)（`AppShell` 网格 + `Sidebar` 可扩展侧边栏 + `StatusBar` 右侧路由区顶状态栏 + `SettingsPage` 设置页 + 主题对照 `ThemePalettePage`＋`installThemedRoot` 令牌根注入；壳组件 CSS Modules + clsx、`var(--bk-*)`，只依赖 `@berkshire/ui-slots`/`react-router-dom`/`clsx`，路由与桥接态由宿主 prop 注入；壳帧 `RootShell` 经共享 `root` 槽挂载，`registerRootShell` 注册即效应）· 布局挂点（`@berkshire/ui-slots` `FrontendSlotContextMap`）增 `layout.navigation.extra`/`layout.sidebar.footer`/`layout.statusbar.right`/`settings.cards`（与 core `SLOT_NAMES` 两端同一契约）＋侧边栏按 `RouteDescriptor.section` 分组 · demo 插件证明性注册（导航追加项/状态项/设置卡片） | + stage `page.header` 每页头槽（v-next）/ shell 折叠态持久化 / store 作用域 / 经 sidecar 装配可 disable（step 3b 余下） |
| demo 插件（T3，能力块 A/B/C 一体化） | [packages/plugins/demo/src/index.ts](../packages/plugins/demo/src/index.ts)（`@berkshire/plugin-demo`：footer + toolbar 组件 + 资金流向页 + 应用壳布局组件 + scoped 样式，`inject: ['slots','clientModules','log']`）· [packages/bundle/demo/cordis.patch.yml](../packages/bundle/demo/cordis.patch.yml)（enable）+ [packages/bundle/demo-off/cordis.patch.yml](../packages/bundle/demo-off/cordis.patch.yml)（disable）· [packages/plugins/demo/examples/smoke.ts](../packages/plugins/demo/examples/smoke.ts)（boot 级 enable/disable 复现）· **webview 半身随插件包走**：[packages/plugins/demo/src/client/](../packages/plugins/demo/src/client/demoFundFlow.tsx)（`DemoFundFlow`/`DemoWatchlistToolbar`/`DemoMoneyFlow` + **应用壳布局组件** `DemoNavExtra`/`DemoStatusItem`/`DemoSettingsCard`，前端 scoped 样式为 **CSS Modules**：作者源 `*.module.css` → 插件打包阶段编译 [scripts/compile-styles.ts](../packages/plugins/demo/scripts/compile-styles.ts) 产出 [styles.generated.ts](../packages/plugins/demo/src/client/styles.generated.ts)，`.tsx` 拿哈希类名、sidecar 拿注入 css 经 `client/list` 下发）。**M3 起 host 不再 static import 插件 client**：插件在运行期自报 `CLIENT_ENTRY_URL`（`new URL('./client/index.js', import.meta.url).href`），sidecar `client/list` 快照携带它，Rust `to_bk_url` 按 `$BK_HOME` 规范化成 `bk:///…`，webview [src/client/loader.ts](../apps/berkshire-agent/src/client/loader.ts)（`importClientModule`）运行时 `import(url)` + 具名导出 + scoped 样式注入（[ClientModuleHost.tsx](../apps/berkshire-agent/src/client/ClientModuleHost.tsx)），共享裸名经 [src/lib/sharedImportMap.ts](../apps/berkshire-agent/src/lib/sharedImportMap.ts) import-map 解析（宿主接线点，默认空）。sidecar 装配叠入 demo bundle（`packages/sidecar/src/index.ts`），`process.test.ts` 断言卸除顺序 `demo -> notify-console -> core` | + HMR / 生产样式分发包 / import-map 真实 URL（待运行验证） |
...
| 类型化事件 / 服务增强（已分散 co-locate，B 级，M4 迁 scoped） | 各服务/事件 `declare module '@berkshire/cordis'` **co-locate 到所属文件**：[services/log.ts](../packages/core/src/services/log.ts)（`ctx.log`）· [services/capabilities.ts](../packages/core/src/services/capabilities.ts)（`ctx.capabilities` + `capabilities/changed`）· [seams/notify.ts](../packages/core/src/seams/notify.ts)（`ctx.notifier` + `notify/request`）· [services/slots.ts](../packages/core/src/services/slots.ts)（`ctx.slots`）· [services/clientModules.ts](../packages/core/src/services/clientModules.ts)（`ctx.clientModules`）· [events.ts](../packages/core/src/events.ts)（跨服务共享事件 `client/changed`，Slots 与 ClientModules 共同发出）· 根入口 [index.ts](../packages/core/src/index.ts) 仍 `import './events'` + 全量 re-export 保证 `import '@berkshire/core'` 一次性激活；**子路径导出已开**（[package.json](../packages/core/package.json) `exports`：`./services/*`/`./seams/notify`/`./events`/`./brand`/`./types`/`./core`），可按服务颗粒独立 import（示例 [packages/core/examples/subpath-probe.ts](../packages/core/examples/subpath-probe.ts)） | 已迁到 scoped `@berkshire/cordis`（M4，见「Cordis 底座」行） |
