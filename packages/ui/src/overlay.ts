 ...
 *
 * 本文件把 `Dialog`/`Sheet`/`Drawer` 共用的「焦点陷阱 + 关闭还原焦点 + ESC 关闭 + body 滚动锁」拆成
 * 可复用的 hooks（机制承自早期 `Modal`，现由 `Dialog` 家族与滑出面板沿袭），避免每件复制粘贴。
 * 不改 `Popover` 源码；它继续用各自的实现（本 helper 只服务新组件）。
 *
...
/** 面板内可聚焦元素选择器（与 `Dialog` 同款）。 */
export const FOCUSABLE =
  'a[href], button:not([disabled]), input:not([disabled]), select:not([disabled]), textarea:not([disabled]), [tabindex]:not([tabindex="-1"])'

/**
 * 打开时聚焦面板、关闭时还原到先前聚焦元素（对齐 `Dialog` 的可访问性契约）。
...
/**
 * Tab/Shift+Tab 焦点陷阱：把可聚焦元素圈定在面板内（对齐 `Dialog` 契约）。
 */
