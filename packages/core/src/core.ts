 ...
 * `ctx.dataSources` / `ctx.database`。
 *
 * 路径 A 示范：`ctx.log` 已作为**独立插件组件（`Log`）**装载（`ctx.plugin(Log)`），其余八服务
 ...
 * clientModules、datasets、dataSources(seam)、database 九条；market/scheduler 仍是目标态（Todo）。
 */
