 ...
 * 而是点「设置」弹出的 `@berkshire/ui` `Dialog` 模态（`DialogContent` 承载。
 *
 * 结构（左分组 + 右表单）：
...
export function SettingsDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={(next) => !next && onClose()}>
      <DialogContent ariaLabel="设置" className={styles.dialog}>
        <DialogHeader className={styles.header}>
          <DialogTitle className={styles.title}>设置</DialogTitle>
        </DialogHeader>

        <Tabs
          value={active}
          onValueChange={(v) => setActive(v as SettingsGroupId)}
          className={styles.tabs}
        >
          <div className={styles.body}>
            {/* 左：分组列表（库 `TabsList`；通用/模型走 order，插件设固定居后）。 */}
            <TabsList className={styles.sidebar} aria-label="设置分组">
              {GROUPS.map((g) => (
                <TabsTrigger
                  key={g.id}
                  value={g.id}
            ...
            {/* 右栏：当前分组表单（库 `TabsContent` 惰性渲染）+ 底部 action bar。 */}
            <div className={styles.content}>
              <div className={styles.panel}>
                <TabsContent value="general">
                  <GeneralSettingsForm storage={storage} />
                </TabsContent>
                <TabsContent value="model">
                  <ModelSettingsForm storage={storage} />
                </TabsContent>
                <TabsContent value="plugins">
                  <PluginsSettingsPanel storage={storage} />
                </TabsContent>
              </div>
...
function GeneralSettingsForm({
  ...
  const theme = usePersistedField(storage, SETTINGS_NS, 'theme', {
    defaultValue: 'dark',
  });
  const language = usePersistedField(storage, SETTINGS_NS, 'language', {
    defaultValue: 'zh-CN',
  });
  const timezone = usePersistedField(storage, SETTINGS_NS, 'timezone', {
    defaultValue: 'Asia/Shanghai',
  });
  const numFormat = usePersistedField(storage, SETTINGS_NS, 'numFormat', {
    defaultValue: 'zh_CN',
  });
  return (
    <FormShell
      title="通用设置"
      desc="主题、语言、时区与数字格式等通用项（字段变更即持久化到 settings/<key>）。"
    >
      <Select
        label="主题"
        value={theme.value}
...
function ModelSettingsForm({ storage }: { storage: StorageHandle }): ReactNode {
  const provider = usePersistedField(storage, SETTINGS_NS, 'provider', {
    defaultValue: 'deepseek',
  });
  const model = usePersistedField(storage, SETTINGS_NS, 'model', {
  ...
  const keyRef = usePersistedField(storage, SETTINGS_NS, 'keyRef', {
    ...
    commit: (next) => {
      ...
      if (!next.trim())
        return {
          ok: false,
          message: '须填一个环境变量/引用名（如 DEEPSEEK_API_KEY）',
        };
      if (!re.test(next.trim())) {
        return {
          ok: false,
          message:
            '引用名须为环境变量式（大写字母/数字/下划线），禁止填明文密钥值',
        };
      }
      return { ok: true };
    },
  ...
  return (
    <FormShell
      title="模型设置"
      desc="AI 适配器 / Keys / 默认模型（字段变更即持久化到 settings/<key>）。"
    >
      <Select
        label="Provider"
...
function PluginsSettingsPanel({
  ...
  return (
    <FormShell
      title="插件设置"
      desc="由各插件经 settings.section 槽贡献的设置表单面板（装上即出现、卸下即消失）；面板把每项落成 $BK_HOME/state/<插件 ns>/<key>。"
    >
      ...
      <div className={styles.cards}>
        <ExtensionSlot
          name="settings.cards"
          context={{ settingsGroups: DEFAULT_SETTINGS_GROUPS }}
        />
      </div>
    </FormShell>
  );
