 ...
 *
 * S3b：视觉对齐 S1，刷新用 `@berkshire/ui` Button（sm ghost），不再手写裸 button。
 */
...
export const DemoWatchlistToolbar = ({ context }: { context: unknown }): ReactNode => {
  ...
  return (
    <span className={watchlistToolbar.classNames.watchlistToolbar}>
      <span className={watchlistToolbar.classNames.meta}>
        自选看板（demo）：{c.symbols?.length ?? 0} 只 · 视图 {c.viewMode}
      </span>
      <Button size="sm" variant="ghost" onClick={() => c.refresh?.()} title="demo 刷新（占位）">
        ⟳ 刷新
      </Button>
    </span>
  )
