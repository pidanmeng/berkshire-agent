...
import { createFuyaoProvider } from './provider'
 ...
 *
 * - 声明依赖：`inject: ['dataSources', 'log']`（解析而非手工排序）；
 * - 提供：把自己作为 `DataSourceProvider` 经 `ctx.dataSources.register()` 挂入能力缝，
 *   覆盖 realtime / daily / adj_factor / financial（minute 声明未落地）；
...
export async function apply(ctx: Context, config: Config): Promise<() => void> {
  ...
  ctx.log.append('datasource-fuyao', { note: '已注册', datasets: ['realtime', 'daily', 'adj_factor', 'financial', 'minute(未落地)'] })
