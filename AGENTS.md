...
│       └── src/             #   webview: main.tsx, App.tsx + slots/（webview 本地 slot 宿主+反应式订阅，T0）+ client/（ClientModuleHost/loader 模块映射宿主，T1 链路）+ routes/（HashRouter 核心路由 + 插件自声明动态路由/导航，路由契约化）；store/client-plugins 为计划
├── docs/                    # 目标架构文档集（中文）+ diagrams(archify 图) + reference(审计/研究)
└── packages/                # v1：headless 核心脊（@berkshire/core、@berkshire/boot）+ 插件 notify-console + demo（client 插件图：页面/组件/样式）+ bundle/{base,headless,demo,demo-off}；其余服务仍计划
```
...
- 保持「已实现 vs 目标态」诚实：**本文件当前可断言 `apps/berkshire-agent` 骨架 + `packages/` headless 核心脊 v1（`core`/`boot`/`plugins/notify-console`/`bundle`）+ 桥接协议 sidecar **T1**（`packages/sidecar`）+ **T2 的 Rust 宿主桥（`src-tauri/src/bridge.rs`/`sidecar_client.rs`）**+ **T3 的 webview 接线（`apps/berkshire-agent/src/lib/api.ts` 薄客户端 + `components/SidecarPanel.tsx` + `lib/ExtensionBoundary.tsx`，最小证明面）**+ **T0 的 webview 本地 slot 宿主（`src/slots/*`，含 `useSyncExternalStore` 反应式订阅）**+ **T1 的 sidecar 侧 `ctx.slots`/`ctx.clientModules` 能力缝 + `client/list`/`client_list` + webview `ClientModuleHost`（最小 client 插件图链路）**+ **动态路由/页面（能力块 B，路由契约化）：core `Slots.routes()`（任意 slot 的 `route`，含 slot 归属 + 全局 path 唯一）/`CORE_ROUTE_PATHS` + sidecar `routes/list` + Rust `routes_list` + webview `react-router-dom` `HashRouter` 与 `src/routes/{routesStore,RouteSync,ExtensionRoute}`（按 route.slot 渲染对应槽）**+ **T3 的 demo 插件（能力块 A/B/C 一体化）：`packages/plugins/demo`（`@berkshire/plugin-demo`，footer 组件 + toolbar 组件 + 资金流向页，各带 scoped 样式；**webview 半身——页面/组件 JSX + 前端样式——随插件包走，经 `@berkshire/plugin-demo/client` 入口 static import**）+ `packages/bundle/{demo,demo-off}`（enable/disable 开关）+ **dev 态插件热更最小通路（仅 dev：组件走 Vite Fast Refresh；样式/声明经 sidecar `dev_watch.ts` → `dev/reload-requested` → 宿主自动重启 + 推 `client/changed`，release 惰性）**为已实现**；`@berkshire/cordis`、正式 router（layout/menu 全量注入）、store、`bk://` 远程 bundle 等仍为目标态。能力一旦落地，同步把上文由「目标态」改为「已实现」并更新「现状锚点」（见 `docs/architecture.md §11`）。