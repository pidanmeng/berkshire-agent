...
if (args.length > 0 && !writeMode) {
  console.error('verify-archived-agent-notes: usage: bun scripts/verify-archived-agent-notes.ts [--write]')
  process.exit(1)
}
...
// CI 提供其受信任的改动前基线；本地写入与已提交 HEAD 比较。
const baselineRef = process.env.BK_ARCHIVE_BASE_REF ?? 'HEAD'
try {
