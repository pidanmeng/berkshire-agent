...
- **插件 front-end 是「分裂交付」**（`packages/plugins/demo/src/client/`）：
  ...
  - 样式是**纯 CSS 字符串**，单一来源 `styles.ts` → 经 sidecar 注册（`demo/src/index.ts` 的 `style:` 字段）→ `client/list` 快照 → 宿主 `loader.ts` 在**运行时**以 `<style data-bk-module>` 注入。
  - dev 热更里 `dev_watch.ts` 把 `src/client/*.ts`（样式串）交给 sidecar 重启转发；`.tsx` 归 Vite Fast Refresh。
- **结论**：组件的家在「构建时」，样式的家在「运行时字符串」。一切样式治理必须先尊重这条分叉。

## 2. 硬规则（必须遵守）

1. **单一事实源，绝不复制签名**：样式 token / 主题类型只有一个家。要 import，不要抄。
2. **令牌优先，禁用魔法值**：插件 `styles.ts` 一律用 `var(--bk-*)` 引用中枢令牌，**禁止硬编码色值/间距/圆角**（如 `#2e86de`、`8px`）。演示组件也适用。
...
- 建共享包 `@berkshire/theme`（**无 `@berkshire/core` 依赖**，纯令牌 + 类型 + CSS 变量名），作样式层的单一事实源：
  ...
  - 中枢 `App.css` 与插件 `styles.ts` **都从这里 import**；`:root`/`[data-theme]` 上只写 `var(…)` 的赋值。
...
- **Consumer**：webview 样式注入（`loader.ts`）+ 各插件 `styles.ts` 读 `var(--bk-*)`。
...
- [ ] 新增样式/主题是否仍然尊重「组件构建时、样式运行时」的分叉，没破坏 dev 热更（`styles.ts` 走 sidecar 转发）？
