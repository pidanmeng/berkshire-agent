...
### 0. 准备 `.agents/features/` 目录（用完即删、不归档）

- `features/` 只放**当前正在开发/规划**的批次；**用完即删、不归档**。某批次已落地（代码/文档进城）后，把其中需要保留的**持久知识**记入永久文档（`docs/` 或 `.agents/notes/`），然后**删除**该批次 feature 文件——不要留档、不要积累历史快照。
- 拆包产物统一落盘到 `../../features/<FEATURE>`（即仓库根的`.agents/features/<FEATURE>`目录）。若目录不存在则先创建。
